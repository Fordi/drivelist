...
module.exports = (input) => {
  ...
  return _.compact(_.map(input.split(/\n\s*\n/), (device) => {
    ...
    deviceMap[result.device] = _.mapValues(result, (value) => {
      if (!_.isString(value)) {
        return value;
      }
    ...
    if ('mountpoints' in deviceMap[result.device] && !deviceMap[result.device].mountpoints) {
      deviceMap[result.device].mountpoints = [];
    }
    if (result.parent) {
      deviceMap[result.parent].partitions = deviceMap[result.parent].partitions || {};
      deviceMap[result.parent].partitions[result.device] = deviceMap[result.device];
    } else {
      return deviceMap[result.device];
    }
  }));
};
